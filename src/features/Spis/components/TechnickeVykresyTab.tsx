...
export const TechnickeVykresyTab: React.FC<TechnickeVykresyTabProps> = ({ isDark, items, onUpdate, isLocked = false, user }) => {

  const handleUpdate = (index: number, field: string, value: string) => {
    if (isLocked) return;
    const newItems = [...items];
    if (!newItems[index]) {
      newItems[index] = { datum: '', kategoria: '', dodavatel: '' };
    }
    ...
    <div className="p-2 h-full flex flex-col gap-4">
      {/* <div className="mb-4 text-right">
        <button
          ...
          disabled={isLocked}
          className={`px-3 py-1 border border-gray-300 text-gray-700 bg-white rounded text-xs hover:bg-gray-50 transition-all font-semibold ${isLocked ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
          Pridať riadok ručne
        </button>
      </div> */}
